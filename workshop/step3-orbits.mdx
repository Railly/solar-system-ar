...
sun.model = glm::scale(glm::mat4(1.0f), glm::vec3(0.6f));  // Grande y visible
...
earth.model = glm::scale(glm::mat4(1.0f), glm::vec3(0.25f));  // Tamaño medio
...
moon.model = glm::scale(glm::mat4(1.0f), glm::vec3(0.08f));  // Pequeña
...
const float TIME_SCALE = 0.3f;  // 30% velocidad - mucho más observable

// Actualizar mecánicas orbitales con escalado de tiempo
...
void Object::update(float dt, float time) {
  ...
  if (orbitTarget) {
    ...
    glm::vec3 orbit = orbitRadius * orbital_calculation(angle, orbitAxis);
    position = orbitTarget->position + orbit;
  } else {
...
// Velocidades demostrativas (con TIME_SCALE = 0.3)
Sol:    5°/s → Rotación muy lenta
Tierra: 8°/s orbital, 30°/s rotación → Movimiento observable
Luna:   25°/s orbital, 20°/s rotación → Más rápida pero controlada

// Factor de tiempo para control fino
const float TIME_SCALE = 0.3f;  // Ralentiza todo al 30%
scene.update(dt * TIME_SCALE, time * TIME_SCALE);
```
